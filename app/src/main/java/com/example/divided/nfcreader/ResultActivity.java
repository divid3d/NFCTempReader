        ...
        if (this.getIntent().getExtras() != null) {
            ...
            if (tempMeasurements != null) {
                ...
                if (tempMeasurements.size() == 1) {
                    ...
                    SingleTempFragment singleTempFragment = new SingleTempFragment();
                    Bundle fragmentBundle = new Bundle();
                    fragmentBundle.putParcelable("fragment_data", tempMeasurements.get(0));
                    singleTempFragment.setArguments(fragmentBundle);
                    fragmentTransaction.replace(R.id.fragment_container, singleTempFragment);
                    fragmentTransaction.commit();

                } else {
                    FragmentManager fragmentManager = getFragmentManager();
                    FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                    MultiTempFragment multiTempFragment = new MultiTempFragment();
                    Bundle fragmentBundle = new Bundle();
                    fragmentBundle.putParcelableArrayList("fragment_data", tempMeasurements);
                    multiTempFragment.setArguments(fragmentBundle);
                    fragmentTransaction.replace(R.id.fragment_container, multiTempFragment);
                    fragmentTransaction.commit();
                }
            }
        }
